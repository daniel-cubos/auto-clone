describe('Testing resume with apply filters in table', () => {
  beforeEach(() => {
    cy.intercept('GET',
  ...
  it('Apply filters in monday and wednesday', () => {
    ...
    cy.get('.open-filters-button').click().then(() => {
      ...
      cy.get('.out').contains('500,00');
      cy.get('.balance').contains('32.000,00');
    })
  });
  ...
  it('Apply filters in monday and wednesday and category "Vendas"', () => {
    ...
    cy.get('.open-filters-button').click().then(() => {
      ...
      cy.get('.balance').contains('32.500,00');
    })
  });
  ...
  it('Apply filters only max value', () => {
    ...
    cy.get('.open-filters-button').click().then(() => {
      ...
      cy.get('.in').contains('0,00');
      cy.get('.out').contains('500,00');
      cy.get('.balance').contains('500,00');
    })
  });
